...
export default function CompanionDetailClient() {
  ...
  const activeParticipantCount = activeBoards.filter((board) => board.role === "participant").length;
  const isRecruitingOpen = currentRoom ? activeParticipantCount < currentRoom.capacity : false;
  const participantSupplyProgress = useMemo(() => (detailState?.boards ?? []).filter((board) => board.role === "participant").map((board) => ({ userId: board.userId, name: board.name, status: getBoardStatus(board), checkedCount: (detailState?.supplies ?? []).filter((supply) => supply.checkedBy.includes(board.userId)).length })), [detailState]);
  ...
  async function handleJoinToggle() {
    ...
    persistLocalDetail({ ...detailState, participants: [...detailState.participants, { id: `${currentRoom.id}-${viewerId}`, userId: viewerId, name: currentUserName, role: "participant" }], boards: [{ id: `${currentRoom.id}-board-${viewerId}`, userId: viewerId, name: currentUserName, role: "participant", lastActivityAt: new Date().toISOString(), graduatedAt: null, posts: [] }, ...detailState.boards] });
  }
      ...
      <div className={styles.shell}><div className={styles.workspace}><div className={styles.mainColumn}>
        ...
        <section className={`${styles.sectionCard} ${styles.sectionSpanFull}`}>
          <div className={styles.sectionHead}>
            <div>
              <h3 className={styles.sectionTitle}>동행방 안내</h3>
              <p className={styles.sectionDescription}>현재 방의 진행 방식과 핵심 규칙을 한눈에 볼 수 있어요.</p>
            </div>
          </div>
          <div className={styles.commentList}>
            <article className={styles.commentCard}>
              <p className={styles.commentBody}>{currentRoom.summary}</p>
            </article>
            <article className={styles.commentCard}>
              <p className={styles.commentBody}>준비물은 참여자만 확인할 수 있고, 진행자는 각 참여자의 준비도를 {detailState.supplies.length || 0}개 기준으로 확인할 수 있어요.</p>
            </article>
            <article className={styles.commentCard}>
              <p className={styles.commentBody}>진행 기록이 1주일 이상 멈추면 자동으로 휴식 칸으로 이동하고, 완료 후 졸업하면 다시 진행 칸으로 돌아올 수 없어요.</p>
            </article>
          </div>
        </section>
